...
//                     frontendTotal: total.toString(),
//                     storeCreditUsed: storeCreditUsed.toString(),
//                     // REMOVED: items: JSON.stringify(...) ← THIS WAS THE PROBLEM
//                     orderPreviewId: orderPreviewId, // ← ONLY this ID
//                     ...otherMetadata,
//                     orderSummary: JSON.stringify(orderSummary),
//                 },
//             });
...
//     // In StripeService class
//     async handleCheckoutSessionCompleted(session: EnhancedStripeSession) {
//         const userId = session.metadata?.userId;
//         const orderPreviewId = session.metadata?.orderPreviewId; // Get preview ID
...
//         if (!orderPreviewId) {
//             throw new Error("No orderPreviewId in session metadata - cannot create order without full details");
//         }

//         try {
//             // 1. GET THE FULL ORDER DETAILS FROM DATABASE USING PREVIEW ID
//             const orderPreview = await prisma.orderPreview.findUnique({
//                 where: { id: orderPreviewId },
//             });

//             if (!orderPreview) {
//                 throw new Error(`Order preview not found: ${orderPreviewId}`);
//             }

//             // Check if expired
//             if (orderPreview.expiresAt < new Date()) {
//                 throw new Error("Order preview has expired");
//             }

//             // 2. PARSE THE FULL ITEM DETAILS
//             const fullItems = JSON.parse(orderPreview.items);
...
//             const shippingCost = parseFloat(session.metadata?.calculatedShipping || orderPreview.shipping.toString());
//             const storeCreditUsed = parseFloat(session.metadata?.storeCreditUsed || "0");
...
//                 shippingCost,
//                 storeCreditUsed,
//                 fullItems, // Use full items from database
//             );

//             // 4. DELETE THE PREVIEW (optional)
//             await prisma.orderPreview
//                 .delete({
//                     where: { id: orderPreviewId },
//                 })
//                 .catch((e) => console.log("Preview already deleted or not found:", e));

//             // ... rest of your order processing (email, commissions, shipstation, etc.)
//         } catch (error) {
//             console.error("Error handling checkout session completed:", error);
//             // Restore store credit if needed
//             const storeCreditUsed = parseFloat(session.metadata?.storeCreditUsed || "0");
//             if (storeCreditUsed > 0 && userId) {
//                 await prisma.user.update({
//                     where: { id: userId },
...
//         itemsFromPreview: any[] = [], // Changed from itemsFromMetadata to itemsFromPreview
//     ) {
//         try {
//             const sessionDetails = await stripe.checkout.sessions.retrieve(session.id, {
//                 expand: ["line_items.data.price.product", "payment_intent", "shipping_cost", "customer_details"],
...
//             // IMPORTANT: Use items from database preview, NOT from lineItems or metadata
//             // This ensures all items are saved correctly with all details
//             for (const item of itemsFromPreview) {
//                 await prisma.orderItem.create({
//                     data: {
//                         id: `item_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
//                         orderId: order.id,
...
//                         size: item.size,
//                         quantity: item.quantity,
//                         unitPrice: item.finalPrice || item.originalPrice, // Use finalPrice if available
//                         discountedPrice: item.finalPrice || item.originalPrice,
//                     },
//                 });
//             }

//             // Optional: Verify that the number of items matches
//             console.log(`✅ Created order ${order.id} with ${itemsFromPreview.length} items from preview`);
...
export class StripeService {
    ...
    async handleCheckoutSessionCompleted(session: EnhancedStripeSession) {
        const userId = session.metadata?.userId;
        const orderPreviewId = session.metadata?.orderPreviewId;
        ...
        try {
            ...
            // 4. GET ORDER ITEMS FOR FURTHER PROCESSING
            const orderItems = await prisma.orderItem.findMany({
                where: { orderId: order.id },
                include: { product: true },
            });

            // 5. UPDATE EACH PRODUCT'S QUANTITY
            for (const item of orderItems) {
                if (!item.productId) continue;
            ...
            // 7. LINK CHECKOUT SESSION TO ORDER
            await prisma.checkoutSession.update({
                where: { stripeSessionId: session.id },
                data: { orderId: order.id },
            });

            // 8. PROCESS COMMISSIONS
            await this.processCommissions(order);

            // 9. CREATE SHIPSTATION ORDER
            try {
                await shipStationService.createOrder(order.id);
            } catch (shipstationError) {
                console.error("ShipStation error (non-fatal):", shipstationError);
            ...
            // 10. DELETE THE PREVIEW (optional)
            await prisma.orderPreview
                .delete({
                    where: { id: orderPreviewId },
                })
            ...
            console.log(`✅ Order ${order.id} successfully processed with ${orderItems.length} items`);
        } catch (error) {
            console.error("Error handling checkout session completed:", error);
            // Restore store credit if order creation fails
            const storeCreditUsed = parseFloat(session.metadata?.storeCreditUsed || "0");
    ...
    private async createOrderFromSession(session: EnhancedStripeSession, userId: string, shippingInfo: any, shippingCost: number, storeCreditUsed: number, itemsFromPreview: any[] = []) {
        try {
            const sessionDetails = await stripe.checkout.sessions.retrieve(session.id, {
                expand: ["line_items.data.price.product", "payment_intent", "shipping_cost", "customer_details"],
            });
            ...
            for (const item of itemsFromPreview) {
                await prisma.orderItem.create({
                    data: {
                        ...
                        size: item.size,
                        quantity: item.quantity,
                        unitPrice: item.finalPrice || item.originalPrice,
                        discountedPrice: item.finalPrice || item.originalPrice,
                    },
                });
            }
